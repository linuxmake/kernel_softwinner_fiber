...
{	
	...
	//This case must be consistent with IMG gpu so in the userspace.
	if ( buffer->flags&(ION_FLAG_CACHED<<16) ) {
		vma->vm_page_prot = __pgprot_modify(vma->vm_page_prot, L_PTE_MT_MASK, L_PTE_MT_WRITEBACK);
	}	else if (!(buffer->flags & ION_FLAG_CACHED))  {
		vma->vm_page_prot = pgprot_writecombine(vma->vm_page_prot); // bufferable
	}
