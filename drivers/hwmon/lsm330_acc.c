...
static int gsensor_fetch_sysconfig_para(void)
{
	...
	if (1 == device_used) {
		...
		printk("lsm330_acc %s: twi_id is %d. \n", __func__, twi_id);
		ret = 0;
...
static int gsensor_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	int ret;
	printk("lsm330_acc gsensor_detect\n");
	...
	if (twi_id == adapter->nr) {
		for (i2c_num = 0; i2c_num < (sizeof(i2c_address)/sizeof(i2c_address[0]));i2c_num++) {
			...
			ret = i2c_smbus_read_byte_data(client,LSM330_WHO_AM_I);
			if ((ret &0x00FF) == WHOAMI_LSM330_ACC) {
				pr_info("lsm330dlc_acc detected\n");				
				strlcpy(info->type, LSM330_ACC_DEV_NAME, I2C_NAME_SIZE);	
				return 0;
			}
			else
			{
				printk("not LSM330DLC_ACC!!\n");
			}
		}
		pr_info("%s:lsm330dlc_acc Device not found, \
...
//			LSM330_ACC_DEV_NAME, hw_d[0]/16, hw_d[1]/16, hw_d[2]/16);
//printk("sensitivity:[%d]", acc->sensitivity);
					...
					int *xyz)
{
	...
	//printk("lsm330_acc_report_values x:[%#x] y:[%#x] z:[%#x]\n",xyz[0],xyz[1],xyz[2]);
}
...
#ifdef DEBUG
		pr_err("invalid interrupt setting, val: %#x\n",val);
#endif
		return -EINVAL;
	}
