...
/* Addresses to scan */
static const unsigned short normal_i2c[] = {0x6a,I2C_CLIENT_END};
static __u32 twi_id = 0;
static const unsigned short i2c_address[2] = {0x6a,0x6b};
static first = 0;
...
static struct lsm330_gyr_platform_data default_lsm330_gyr_pdata = {
	.fs_range = LSM330_GYR_FS_2000DPS/*LSM330_GYR_FS_250DPS*/,
	.axis_map_x = 0,
	.axis_map_y = 1,
	.axis_map_z = 2,
	.negate_x = 1,
...
static int gyr_fetch_sysconfig_para(void)
{
	...
	device_used = val.val;
	device_used = 1;//gongpiqiang,在代码里面直接赋值，修改太多fex比较麻烦
...
static int gyr_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	printk("lsm330_gyr detect\n");
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA))
		return -ENODEV;
printk("----------twi_id:[%d----------]",twi_id);
	if(twi_id == adapter->nr){
		for(i2c_num = 0; i2c_num < (sizeof(i2c_address)/sizeof(i2c_address[0]));i2c_num++)
		{		
			client->addr = i2c_address[i2c_num];
#ifdef DEBUG
			printk("lsm330 check i2c addr: %#x .\n", client->addr);
#endif
			ret = i2c_smbus_read_byte_data(client,WHO_AM_I);

			pr_info("Lsm330 gyr Read ID value is :%#x",ret);
			if ((ret &0x00FF) == WHOAMI_LSM330_GYR) {
				pr_info("lsm330 Device detected!\n" );
    				strlcpy(info->type, LSM330_GYR_DEV_NAME, I2C_NAME_SIZE);
				return 0; 
			}
		}
		pr_info("%s:lsm330 Device not found, \
...
static void lsm330_gyr_device_power_off(struct lsm330_gyr_status *stat)
{
	int err;
	u8 buf[2];
printk("lsm330_gyr_device_power_off\n");
...
printk("lsm330_gyr probe success\n");
