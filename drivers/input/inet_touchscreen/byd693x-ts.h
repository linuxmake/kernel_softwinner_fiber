...
//#define CONFIG_LANDSCAPE_SCREEN 1 //Paul added 
...
struct sTYPEID_Info
{
	unsigned char	sTYPEID_TYPEH;		//型号数字	2字节
	unsigned char	sTYPEID_TYPEL;
	unsigned char	sTYPEID_CoverID;	//面板代号,F--  P--  G--
	unsigned char	sTYPEID_DateNo[5];	//日期序列号，为EXCEL计算出来的日期序列号
	unsigned char	sTYPEID_ChipNo[8];	//芯片序列号，
	unsigned char	sTYPEID_Type_Str[11];		//型号的字符串，以0结尾
	unsigned char	sTYPEID_FPC_Str[9];		//FPC型号的字符串，以0结尾
	unsigned char	sTYPEID_Chip_Str[6];		//芯片型号的字符串，以0结尾
	unsigned char	sTYPEID_Ver_Str[5];		//软件版本型号的字符串，以0结尾
	unsigned char	sTYPEID_Reserve_Str[9];		//预留信息
	unsigned char sTEST_MODE_MAX;
	unsigned char sTEST_SAMPLE_TIMES;
//	unsigned short sTYPEID_CFG_RESO_X;		//默认的分辨率 X
//	unsigned short sTYPEID_CFG_RESO_Y;		//默认的分辨率 Y
	...
	unsigned char	sTYPEID_IIC_Address;		//IIC 地址
};
...
{ 1, 0x07, 0x01, 0x00},	// Wakeup TP from Sleep mode	//Charles 12-12-10 change from 1 to 0
};
