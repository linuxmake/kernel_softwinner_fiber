...
static void elan_ts_touch_down(struct elan_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
	...
	x = x * screen_max_x/ts->y_resolution;
	y = y * screen_max_y/ts->x_resolution;
...
static int elan_ts_recv_data(struct elan_ts_data *ts, uint8_t *buf)
{
	...
	{
		test = 1;
		rc = i2c_master_recv(ts->client, buf, 4);
		printk("[elan] elan_ts_recv_data  hello packet\n");
			
		if(4 != rc){
			printk("[elan error] elan_ts_recv_data  hello packet\n");
			return -1;
		}
	}
...
static int ctp_get_system_config(void)
{
	...
	screen_max_x = config_info.screen_max_y;
	screen_max_y = config_info.screen_max_x;
