 ...
 *  for this touchscreen to work, it's slave addr must be set to 0x7e | 0x70
	2013-04-22_09:20
		modify init time and test power on the machine ,touch IC is not used.

	2013-04-22_10:53
	       modify msleep for test (mdelay)

	2013-06-11_12:06
	      modify driver for K80 800*600 1024*768 KQ70 1024*600 800*480
 */
#include <linux/i2c.h>
...
#define FT5X02_IC_INFO  "\n============================================================== \
						 \nIC     :Focal ft5x02  \
                         \nAUTHOR :p.huan@163.com \
                         \nVERSION:2013-06-11_12:06\n"
...
static const int chip_id_value[CHIP_ID_VALUE_NUM] = {0x02};
...
static int ctp_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(twi_id == adapter->nr)
	{
          ...
          printk("ft5x02 addr:0x%x,chip_id_value:0x%x\n",client->addr,ret);
...
static void ft5x_report_multitouch(void)
{
	...
	case 4:
		if((event->x4>4000)&&(event->y4>4000))  		
					return;
	...
	case 3:
		if((event->x3>4000)&&(event->y3>4000))  		
					return;
	...
	case 2:
		if((event->x2>4000)&&(event->y2>4000))  		
					return;
	...
	case 1:
		if((event->x1>4000)&&(event->y1>4000))  		
					return;
