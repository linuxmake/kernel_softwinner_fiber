 ...
 *  for this touchscreen to work, it's slave addr must be set to 0x7e | 0x70e 

     2013-06-17_19:55
     modify 13.1 machine tp updat file,the version is 0x12
 */
#include <linux/i2c.h>
...
#define FT5X06_IC_INFO  "\n============================================================== \
											   \nIC     :Focal ft5x06 \
                         \nAUTHOR :mbgalex@163.com \
                         \nVERSION:2013-06-17_19:55\n"
...
static int ctp_focal_upgrade = 0;
//static int ctp_cob = 0;

static int head_file_len;
static unsigned char *CTPM_FW;

static unsigned char CTPM_FW_NORMAL[]=
{
#include "ft_app.i"
};
...
static unsigned char CTPM_FW_Q1301[]=
{
//#include "IT_iNet_Q130A_13.3_V10_1280X800_20130504_app.i"
//#include "IT_iNet_Q130A_13.3_V10_1280X800_20130614_app.i"
//#include "IT_iNet_Q130A_13.3_V12_1280X800_20130614_app.i"
//#include "IT_iNet_Q130A_13.3_V13_1280X800_20130614_app.i"
//#include "IT_iNet_Q130A_13.3_V15_1280X800_20130821_app.i"
#include "IT_iNet_Q130A_13.3_V16_1280X800_20130828_app.i"
};


static unsigned char CTPM_FW_Q1302[]=
{
//#include "IT_iNet_Q130A_13.3_V17_1280X800_20130828_app_QSD.i"
//#include "IT_iNet_Q130_13.3_1280X800_V18_20130902_app.i"
#include "IT_iNet13001_app_20130914V2.i"
};

static unsigned char CTPM_FW_Q1303[]=
{
#include "IT_iNet_Q130_jianzhan_13.3_1280X800_V19_20130903_app.i"
};


static unsigned char CTPM_FW_Q1201[]=
{
//#include "IT_iNet_Q130B_11.6'_V10_1366X768_20130506_app.i"
#include "IT_iNet_Q130B_11.6_V11_1366X768_20130521_app.i"

};

static unsigned char CTPM_FW_Q1201_QSD[]=
{
#include "IT_iNet_Q120_11.6_1366X768_V12_20130823_app_qsd.i"

};


/* Addresses to scan */
static const unsigned short normal_i2c[2] = {FT5X06_I2C_ADDR,I2C_CLIENT_END};
#define CHIP_ID_VALUE_NUM 3
static const int chip_id_value[CHIP_ID_VALUE_NUM] = {0x55,0x06,0x08};//{0x55,0x06,0x08,0x02,0xa3};
...
static int inet_getChipA3(struct i2c_client *client)
{

		int ret = -1;
		unsigned char buf_A3[2]={0xA3};
		
		msleep(500);
		
		ret=ft5x_i2c_rxdata(buf_A3, 2);
		...
		printk("ft5x06 getA3=0x%x\n",buf_A3[0]);
		
		if(buf_A3[0]==0x2 || buf_A3[0]==0x3)
		{
			printk("This is not ft5x06 chip,so quit!\n");
			return 0;
		}
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(u8* pbt_buf, u16 dw_lenth)
{
        ...
        do{
         ...
         }while(reg_val[0] != 0x79 || reg_val[1] != 0x03);//while((reg_val[1] != 0x03)&&(reg_val[1] != 0x06));//while(reg_val[0] != 0x79 || reg_val[1] != 0x03);
...
#define BUFFER_LEN (2)            //len == 2 
	...
	/*
	  ...
	  */
	//if((ctp_cob == 1 && ctp_focal_upgrade == 1201) || (ctp_cob == 1 && ctp_focal_upgrade == 1202) || (ctp_cob == 1 && ctp_focal_upgrade == 1301)|| (ctp_cob == 1 && ctp_focal_upgrade == 1302) || (ctp_cob == 1 && ctp_focal_upgrade == 1303))
	if((ctp_focal_upgrade == 1201) || (ctp_focal_upgrade == 1202) || (ctp_focal_upgrade == 1301)|| (ctp_focal_upgrade == 1302) || (ctp_focal_upgrade == 1303))
	{
		if ( 0xa6 == a ||a != b )
		{
			/*call the upgrade function*/
			i_ret =  fts_ctpm_fw_upgrade(&pbt_buf[0],head_file_len);
			if (i_ret != 0){
...
static int ft5x_read_data(void)
{
	...
	data_length = 3 + 4*event->touch_point + 2*(event->touch_point -1);
	if ((event->touch_point == 0)||(event->touch_point >10)) {
		ft5x_ts_release();
		return 1; 
	}
	buf[0] = 0x00;
	ret = ft5x_i2c_rxdata(buf, data_length);
	if (ret < 0) {
...
static void ft5x_ts_late_resume(struct early_suspend *handler)
{
	...
	ft5x_ts_release();//Paul added 20130117
...
static void ft5x06_upgrade(void)
{
int update_flag = 0;

#ifdef CONFIG_SUPPORT_FTS_CTP_UPG
	printk("=====upgrade ft_app.i====");
	head_file_len = sizeof(CTPM_FW_NORMAL);
	CTPM_FW = CTPM_FW_NORMAL;

	update_flag = 1;
#else
	
switch (ctp_focal_upgrade)
	{
		
	
		case 1301:
				printk("======CTPM_FW_Q1301.i======\n");
				printk("======Download V16=========\n");
				head_file_len = sizeof(CTPM_FW_Q1301);
				CTPM_FW = CTPM_FW_Q1301;
				update_flag = 1;			
				break;
		case 1302:
				head_file_len = sizeof(CTPM_FW_Q1302);
				CTPM_FW = CTPM_FW_Q1302;
				update_flag = 1;
				break;
		case 1303:
				head_file_len = sizeof(CTPM_FW_Q1303);
				CTPM_FW = CTPM_FW_Q1303;
				update_flag = 1;
				break;
		case 1201:
			  printk("======CTPM_FW_Q1201.i======\n");
				head_file_len = sizeof(CTPM_FW_Q1201);
				CTPM_FW = CTPM_FW_Q1201;
				update_flag = 1;
				break;
		case 1202:
			  printk("======CTPM_FW_Q1201_QSD.i======\n");
				head_file_len = sizeof(CTPM_FW_Q1201_QSD);
				CTPM_FW = CTPM_FW_Q1201_QSD;
				update_flag = 1;
				break;
		default:
				//printk("=====upgrade ft_app.i====");
...
{   
        ...
        //if (ctp_cob >= 1)
        {
        	if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("ctp_para", "ctp_focal_upg", &val)){
        	ctp_focal_upgrade = 0;
	        }
	        else
	        	ctp_focal_upgrade = val.val;

			//ctp_focal_upgrade =1301;
	        printk("ctp_focal_upgrade = %d\n", ctp_focal_upgrade);
        }
