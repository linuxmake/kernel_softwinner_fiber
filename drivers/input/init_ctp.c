...
#endif
				if(config_info.power_gpio_number)	
						gpio_free(config_info.power_gpio_number);
				ctp_power_en(0); //Paul added @20130116
	return;
}
...
{	
	...
	if(SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		...
		type = script_get_item("ctp_para", "ctp_power_en", &item);
		if(SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
			printk("script_get_item ctp_power_en err\n");
		}
		else
		{
			config_info.power_gpio_number = item.gpio.gpio;
			printk("script_get_item ctp_power_en ok!!\n");
		}                            
	return 0;
}
...
void ctp_print_info(struct ctp_config_info info,int debug_level)
{
       if(debug_level == DEBUG_INIT)
       {
                ...
                dprintk(DEBUG_INIT,"info.ctp_cob:%d\n",info.ctp_cob);
...
static int ctp_fetch_sysconfig_para(void)
{
	...
	config_info.screen_max_x = val.val;
        ...
        if(SCIRPT_ITEM_VALUE_TYPE_INT != script_get_item("ctp_para", "ctp_cob", &val)){
        	config_info.ctp_cob = 0;
        }
        else
        	config_info.ctp_cob = val.val;
        printk("ctp_cob = %d\n",config_info.ctp_cob);
...
//mbg ++ 2013-01-25_15:20
int ctp_wakeup_inet(int status,int ms)
{
       u32 gpio_status;
       dprintk(DEBUG_INIT,"***CTP*** %s:status:%d,ms = %d\n",__func__,status,ms); 
       //printk("\n\n[FUN]%s: status = %d,  ms = %d\n",__func__,status,ms);
        ...
        mdelay(ms);
        //printk("chip status is %d,and delay is %d",status,ms);
...
/**
 * ctp_power_en - function
 *
 */
int ctp_power_en(int status)
{
       u32 gpio_status;
       if(!config_info.power_gpio_number)
       {
       		printk("Paul %s no need !!!!",__func__);
       		return 0;
       }       	

       dprintk(DEBUG_INIT,"***CTP*** %s:status:%d,ms = %d\n",__func__,status); 
        
        gpio_status = sw_gpio_getcfg(config_info.power_gpio_number);
        if(gpio_status != 1){
                sw_gpio_setcfg(config_info.power_gpio_number,1);
        }
        if(status == 0){                
             __gpio_set_value(config_info.power_gpio_number, 0);
        }
        if(status == 1){
            __gpio_set_value(config_info.power_gpio_number, 1); 
         }      
        msleep(10);  
	return 0;
}
EXPORT_SYMBOL(ctp_power_en);
...
static int __init ctp_init(void)
{
	...
	if (ctp_fetch_sysconfig_para()){
        ...
        ctp_power_en(1);//Paul add for power_en %d
        return 0;
}
