...
extern  __bool __raw_sensor__(int dev_id, int camera_num, char *back_name, char *front_name, int front_back);
#endif
...
//gongpiqiang+++
#if 1
int fetch_inet_config(struct vfe_dev *dev)
{  
  int ret;
  unsigned int i;
  char vfe_para[16] = {0};
  char dev_para[16] = {0};
  int camera_num,front_back;
  ...
  camera_export_type_info(&camera_num, back_name, front_name, &front_back);
  printk("fetch_inet_config:camera_num=%d,back_name=%s,front_name=%s,front_back=%d\n",camera_num,back_name,front_name,front_back);
	dev->dev_qty = camera_num;
  for(i=0; i<dev->dev_qty; i++)
  {
    /* i2c and module name*/
    sprintf(dev_para, "vip_dev%d_twi_id", i);
    ...
    //20130801 wzh +++
	if(__raw_sensor__(i, dev->dev_qty, back_name, front_name, front_back))
	{
		vfe_dbg(0, "dev[%d] raw_sensor detected use is_isp_used.\n", i);
		printk("dev[%d] raw_sensor detected use is_isp_used.\n", i);
		dev->ccm_cfg[i]->is_isp_used  = 1;
		dev->ccm_cfg[i]->is_bayer_raw = 1;		
	}//20130801 wzh ---
	else
	{
	    /* isp used mode */
    ...
    /* fetch flip issue */
    sprintf(dev_para, "vip_dev%d_%s_vflip", i, i==0 ? back_name : front_name);
   	if(dev->dev_qty == 1 && front_back == 1)//single front camera
   		sprintf(dev_para, "vip_dev1_%s_vflip", front_name);
    type = script_get_item(vfe_para, dev_para, &val);
    if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
      vfe_dbg(0,"fetch vip_dev%d_vflip from sys_config failed\n", i);
      //
      sprintf(dev_para, "vip_dev%d_%s_vflip", i, i==0 ? "gc2035" : "gc0308");
	   	if(dev->dev_qty == 1 && front_back == 1)//single front camera
	   		sprintf(dev_para, "vip_dev1_%s_vflip", "gc0308");
	    type = script_get_item(vfe_para, dev_para, &val);
			if (SCIRPT_ITEM_VALUE_TYPE_INT == type)
				dev->ccm_cfg[i]->vflip = val.val;      
    ...
    sprintf(dev_para, "vip_dev%d_%s_hflip", i, i==0 ? back_name : front_name);
    if(dev->dev_qty == 1 && front_back == 1)//single front camera
   		sprintf(dev_para, "vip_dev1_%s_hflip", front_name);
    type = script_get_item(vfe_para, dev_para, &val);
    if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
      vfe_dbg(0,"fetch vip_dev%d_hflip from sys_config failed\n", i);
      sprintf(dev_para, "vip_dev%d_%s_hflip", i, i==0 ? "gc2035" : "gc0308");
    	if(dev->dev_qty == 1 && front_back == 1)//single front camera
   			sprintf(dev_para, "vip_dev1_%s_hflip", "gc0308");
 	    type = script_get_item(vfe_para, dev_para, &val);
 	    if (SCIRPT_ITEM_VALUE_TYPE_INT == type)
 	         dev->ccm_cfg[i]->hflip = val.val;
    } else {
    ...
    if (SCIRPT_ITEM_VALUE_TYPE_STR != type){
      ...
      strcpy(dev->ccm_cfg[i]->dvdd_str, val.str);
		...
		sprintf(dev_para, "vip_dev%d_dvdd_vol", i);
		if (strcmp(back_name,"ov5640") == 0)//ov5640
    	sprintf(dev_para, "vip_dev0_ov5640_dvdd_vol");
    type = script_get_item(vfe_para,dev_para, &val);
		if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
	    dev->ccm_cfg[i]->power.dvdd_vol=0;
			vfe_dbg(0,"fetch vip_dev%d_dvdd_vol from sys_config failed, default =0\n",i);
    ...
    sprintf(dev_para, "vip_dev%d_reset", i);
    if(dev->dev_qty == 1 && front_back == 1)//single front camera
    	sprintf(dev_para, "vip_dev1_reset");
    type = script_get_item(vfe_para, dev_para, &val);
    if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
      dev->ccm_cfg[i]->gpio.reset_io.gpio = GPIO_INDEX_INVALID;
      vfe_dbg(0,"fetch vip_dev%d_reset from sys_config failed\n", i);
    ...
    sprintf(dev_para, "vip_dev%d_pwdn", i);
    if(dev->dev_qty == 1 && front_back == 1)//single front camera
    	sprintf(dev_para, "vip_dev1_pwdn");
    type = script_get_item(vfe_para, dev_para, &val);
    if (SCIRPT_ITEM_VALUE_TYPE_PIO != type){
      dev->ccm_cfg[i]->gpio.pwdn_io.gpio = GPIO_INDEX_INVALID;
      vfe_dbg(0,"fetch vip_dev%d_stby from sys_config failed\n", i);
    ...
    if ((SCIRPT_ITEM_VALUE_TYPE_INT == type) && (val.val == 1)) 
	{
      //camera_export_info(dev->ccm_cfg[i]->ccm, &dev->ccm_cfg[i]->i2c_addr,i);
		if((dev->dev_qty == 1) && (front_back == 1))//single front camera
			{
				printk("single camera\n");
			camera_export_info(cam_name, &address,1);
		}
		else
			camera_export_info(cam_name, &address,i);
		printk("i=%d,cam_name=%s,address=0x%x\n",i,cam_name,address);
  ...
  for(i=0; i<dev->dev_qty; i++)
  {
    ...
    vfe_dbg(0,"dev->ccm_cfg[%d]->act_name = %s\n",i,dev->ccm_cfg[i]->act_name);
    vfe_dbg(0,"dev->ccm_cfg[%d]->act_slave = 0x%x\n",i,dev->ccm_cfg[i]->act_slave);
	vfe_dbg(0,"dev->ccm_cfg[%d]->is_isp_used = 0x%x\n",i,dev->ccm_cfg[i]->is_isp_used);
	vfe_dbg(0,"dev->ccm_cfg[%d]->is_bayer_raw = 0x%x\n",i,dev->ccm_cfg[i]->is_bayer_raw);
  }
