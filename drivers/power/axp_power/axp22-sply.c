...
struct delayed_work usbacwork;//Rocky@20130523+
#ifdef CONFIG_HAS_EARLYSUSPEND
            ...
            union power_supply_propval *val)
{
  ...
  if (charger->bat_det) {
	...
	enable_cmd = 1;
	 //_write_charge_enable_cmd(); //zhongwei++ zet6221_tp charge_mode_disable cmd write
    	} else if(charger->charge_on) {
    		val->intval = POWER_SUPPLY_STATUS_CHARGING;
		enable_cmd = 1;
		 //ts_write_charge_enable_cmd(); //zhongwei++ zet6221_tp charge_mode_enable cmd write
    	} else {
    		val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
		enable_cmd = 0;
	//	ts_write_charge_disable_cmd(); //zhongwei++ zet6221_tp charge_mode_disable cmd write
    } 
}
    else {
...
static void axp_change(struct axp_charger *charger)
{
  ...
  if(axp_debug){
	...
	schedule_delayed_work(&usbacwork, msecs_to_jiffies(2* 1000));//Rocky@20130523+
...
{
   int ret;
   int pmu_ntc_init = 0;//gongpiqiang+++
   uint8_t val;
  ...
  //gongpiqiang+++ for bat support ntc
  ret = axp_script_parser_fetch("pmu_para", "pmu_ntc_init", &pmu_ntc_init, sizeof(int));
  if(pmu_ntc_init == 1) {
  	axp_update(charger->master, 0x82, 0x01 , 0x01); //TS管脚ADC功能使能
    
		axp_update(charger->master, 0x84, 0x10 , 0x30); //设置为40uA输出电流
		axp_update(charger->master, 0x84, 0x02 , 0x07); //TS管脚设置为电池温度检测功能,管脚电流输出方式设置为ADC采样时输入
		val = 0x2b;//0x28->60 度  0x2b->55 度
		axp_write(charger->master, 0x39, val);//高温充电控制
		val = 0xa4;//-10 度
		axp_write(charger->master, 0x38, val);//低温充电控制
		val = 0x28;
		axp_write(charger->master, 0x3d, val);//高温放电控制
		val = 0xa4;
		axp_write(charger->master, 0x3c, val);//低温放电控制
  }
  //gongpiqiang--- for bat support ntc
  return ret;
}
...
//	uint16_t tmp;
	...
	//gongpiqiang+++
	/*
	if (charger->ext_valid && (charger->is_on == 0)) {//not charging but ac or usb on
		if(charger->rest_vol!=100) {
			charger->rest_vol = 100;
...
//Rocky@20130523+
static void axp_usbac(struct work_struct *work)
{
	if(axp_charger->ext_valid){
		printk("usb/ac insert enable charging\n");
  		axp_set_bits(axp_charger->master,AXP22_CHARGE_CONTROL1,0x80);
		
	}else{
		printk("usb/ac remove disable charging\n");
  		axp_clr_bits(axp_charger->master,AXP22_CHARGE_CONTROL1,0x80);
	}
}
//Rocky@20130523-
...
//probe 时初始化RDC，使其提前计算正确的OCV，然后在此处启动计量系统
  ...
  /* set usb cur-vol limit*/
	...
	INIT_DELAYED_WORK(&usbacwork, axp_usbac);//Rocky@20130523+
...
static int axp22_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	cancel_delayed_work_sync(&charger->work);
	cancel_delayed_work_sync(&usbwork);
	cancel_delayed_work_sync(&usbacwork);//Rocky@20130523+
...
EXPORT_SYMBOL_GPL(enable_cmd);
EXPORT_SYMBOL_GPL(disable_cmd);
