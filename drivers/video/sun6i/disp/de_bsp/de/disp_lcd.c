...
static int lcd_ext_eldo3 = 0;//gongpiqiang+++
static int lcd_ext_dldo1 = 0;//gongpiqiang+++
__s32 LCD_POWER_DC1SW_EN(__u32 sel, __bool b_en);//gongpiqiang+++
int lcd_id_used = 0;//gongpiqiang+++20140312
int lcd_id = 0;//gongpiqiang+++20140312
int tcon_fix = 0;//gongpiqiang+++20140324
...
__s32 LCD_parse_panel_para(__u32 sel, __panel_para_t * info)
{
    ...
    //gongpiqiang+++
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_ext_eldo3", &value, 1);
    if(ret < 0)
    {
    	 lcd_ext_eldo3 = 0;
        DE_WRN("fetch script data %s.lcd_ext_eldo3 fail\n", primary_key);
    }
    else
    {
        lcd_ext_eldo3 = value;
        printk("lcd_ext_vdd = %d\n", value);
    }
    
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_ext_dldo1", &value, 1);
    if(ret < 0)
    {
    	 lcd_ext_dldo1 = 0;
        DE_WRN("fetch script data %s.lcd_ext_dldo1 fail\n", primary_key);
    }
    else
    {
        lcd_ext_dldo1 = value;
        printk("lcd_ext_dldo1 = %d\n", value);
    }   
...
// for test
   ...
   */
   //info->lcd_x = 720;
   //info->lcd_y = 1024;
//for test ,gongpiqiang
//gongpiqiang+++ for lcd_id 20140313
    __hdle lcd_id_hdl;
    char sub_name[25];
    int i;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_id_used", &lcd_id_used, 1);
		if(lcd_id_used)
			{
				disp_gpio_set_t  gpio_info[1];
				for(i=0;i<lcd_id_used;i++)
					{
						sprintf(sub_name, "lcd_gpio_id%d", i);
						printk("sub_name=%s\n",sub_name);
		        ret = OSAL_Script_FetchParser_Data(primary_key,sub_name, (int *)gpio_info, sizeof(disp_gpio_set_t)/sizeof(int));
		        if(ret ==0)
		        	{
		        		printk("gpio_info->pull=%d\n",gpio_info->pull);
		        		gpio_info->pull=1;
				        lcd_id_hdl = OSAL_GPIO_Request(gpio_info, 1);
				        printk("lcd_id_hdl=%d\n",lcd_id_hdl);
				        printk("read:%d\n",OSAL_GPIO_DevREAD_ONEPIN_DATA(lcd_id_hdl, sub_name));
				        if(OSAL_GPIO_DevREAD_ONEPIN_DATA(lcd_id_hdl, sub_name))
				        	lcd_id = lcd_id + (1 << i);
				      }
	      	}
	      	printk("lcd_id=%d\n",lcd_id);
...
//gongpiqiang--- for lcd_id 20140313
		//gongpiqiang+++20140324
  	ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_tcon_fix", &value, 1);
    if(ret == 0)
    {
        tcon_fix = value;
    }
...
//        channel, pwm_info->enable, pwm_info->active_state, pwm_info->duty_ns, pwm_info->period_ns,pwm_info->mode);
    ...
    //pwm_enable(channel, pwm_info->enable);//gongpiqiang mask,don't need enable here,pwm will enable in LCD_PWM_EN when boot
...
__s32 LCD_PWM_EN(__u32 sel, __bool b_en)
{
    if(gdisp.screen[sel].lcd_cfg.lcd_pwm_used)
    {
        ...
        {   
            ...
            hdl = OSAL_GPIO_Request(gpio_info, 1);
            OSAL_GPIO_Release(hdl, 2);
            if(strcmp("BP101WX1",gpanel_info[sel].lcd_model_name) == 0)
        			LCD_POWER_DC1SW_EN(sel,b_en);
        }       
    }
...
__s32 LCD_BL_EN(__u32 sel, __bool b_en)
{
    ...
    if(gdisp.screen[sel].lcd_cfg.lcd_bl_en_used)
    {
        ...
        if(!b_en)
        {
            ...
            //gpio_info->mul_sel = 7;//gongpiqiang
				if(strcmp("BP101WX1",gpanel_info[sel].lcd_model_name) == 0) 
					{
					bright = BSP_disp_lcd_get_bright(0);
					for(i=0;i<bright/10;i++) {
		            	BSP_disp_lcd_set_bright(0,BSP_disp_lcd_get_bright(0)-10,0);
		            	msleep((255/bright) * 5);
	            	}
				}
        }
...
//DC1SW for vcc-lcd enable func
__s32 LCD_POWER_DC1SW_EN(__u32 sel, __bool b_en)//gongpiqaing+++
{
    __u8 data;
    __u32 ret;
    __u8 addr;
...
__s32 LCD_POWER_EN(__u32 sel, __bool b_en)
{
	...
	if(b_en)
	{
		...
		int ret;
		    ...
		    sprintf(primary_key, "lcd%d_para", sel);
		    ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_reset", (int *)gpio_info, sizeof(disp_gpio_set_t)/sizeof(int));
		    if(ret == 0)
		    {
		    		printk("lcd_reset:%d\n",gpio_info->gpio);
		        gpio_info->data = 0;
		        ...
		        OSAL_GPIO_Release(hdl, 2);
		        msleep(20);
		        printk("gongpiqiang.....................lcd_reset to low\n");
    		}
		//test
		//LCD_BL_EN(sel,FALSE);//gongpiqiang
		LCD_PWM_EN(sel, 0);//close pwm module, gongpiqiang+++
		if(strcmp("BP101WX1",gpanel_info[sel].lcd_model_name) == 0)
			LCD_POWER_DC1SW_EN(sel,1);
		if(gdisp.screen[sel].lcd_cfg.lcd_power_used)
        {
	        ...
	        if(lcd_ext_eldo3 > 0)
	        {
		      	LCD_POWER_ELDO3_EN(sel,1,lcd_ext_eldo3/100);
	        }
	        if(lcd_ext_dldo1>0)
	        {
	        	LCD_POWER_DLDO1_EN(1,1,lcd_ext_dldo1/100);//Rocky+
	        }
        ...
        if(!tcon_fix)//gongpiqiang+++
        Disp_lcdc_pin_cfg(sel, DISP_OUTPUT_TYPE_LCD, 1);
        msleep(10);
        
        //for lcd reset by gongpiqiang
		    /*int ret;
		    char primary_key[20];
		    ...
		    {  
		        ...
		        hdl = OSAL_GPIO_Request(gpio_info, 1);
		        OSAL_GPIO_Release(hdl, 2);
		        printk("gongpiqiang2222.....................lcd_reset\n");
    		}
    		//test
	}
	else
	{
        ...
        if(gdisp.screen[sel].lcd_cfg.lcd_power_used)
        {
	        ...
	        if(lcd_ext_eldo3 > 0)
	        {
		      	LCD_POWER_ELDO3_EN(sel,0,lcd_ext_eldo3/100);
	        }
	        
	        if(lcd_ext_dldo1>0)
	        {
	        	LCD_POWER_DLDO1_EN(sel,0,lcd_ext_dldo1/100);//Rocky+
	      	}
					//gongpiqiang---
        ...
        //for lcd reset by gongpiqiang
		    ...
		    if(ret == 0)
		    {
		        ...
		        hdl = OSAL_GPIO_Request(gpio_info, 1);
		        OSAL_GPIO_Release(hdl, 2);
		        printk("set lcd_reset 0\n");
		        msleep(5);
    		}
    		//for lcd reset by gongpiqiang
    		LCD_POWER_DC1SW_EN(sel,0);//dc1sw在插usb待机时不会关掉，在唤醒时会打开??
	}
    ...
    if(gdisp.screen[sel].lcd_cfg.lcd_used)
    {
        if(gdisp.screen[sel].lcd_cfg.lcd_pwm_used == 1)
        {
           ...
           // pwm_info.duty_ns = (backlight_bright * period_ns) / 256;
            if(gpanel_info[sel].lcd_pwm_pol == 0)
            {
                pwm_info.duty_ns = (backlight_bright * pwm_info.period_ns) / 256;
            }
            else
            {
                pwm_info.duty_ns = ((256 - backlight_bright) * pwm_info.period_ns) / 256;
            }
...
{    
    ...
    //Disp_lcdc_pin_cfg(sel, DISP_OUTPUT_TYPE_LCD, 1);
    //gongpiqiang+++ for tcon fix
    if(tcon_fix) {
		  printk("lcd_tcon_fix = %d\n",tcon_fix);
		  Disp_lcdc_pin_cfg(sel, DISP_OUTPUT_TYPE_LCD, 1);
  	}
    //gongpiqiang--- for tcon fix
    if(gpanel_info[sel].tcon_index == 0)
...
{	    
    ...
    __u64 period_ns;
    if(bright == 0 &&  strcmp("BP101WX1",gpanel_info[sel].lcd_model_name) == 0)//gongpiqiang+++
    	return DIS_SUCCESS;
    bright = (bright > 255)? 255:bright;
    ...
    if((gdisp.screen[sel].lcd_cfg.lcd_pwm_used==1) && (gdisp.screen[sel].lcd_cfg.lcd_used))
    {
        ...
        //printk("[PWM]bright=%d,backlight_dimming=%d, period_ns=%d, duty_ns=%d\n",
...
__s32 BSP_disp_close_lcd_backlight(__u32 sel)
{
    ...
    int ret;
    char primary_key[20];
    int lcd_pwm_pol = 0;//gongpiqiang+++
    char lcd_model_name[20];//gongpiqiang+++
    sprintf(primary_key, "lcd%d_para", sel);
    ...
    {
    	  //gongpiqiang+++
    		OSAL_Script_FetchParser_Data(primary_key, "lcd_pwm_pol", &lcd_pwm_pol, 1);
    		if(lcd_pwm_pol == 0) {
	        gpio_info->mul_sel = 1;
	        gpio_info->data = 0;
	      }
        ...
        hdl = OSAL_GPIO_Request(gpio_info, 1);
        OSAL_GPIO_Release(hdl, 2);
        OSAL_Script_FetchParser_Data(primary_key, "lcd_model_name", (int*)lcd_model_name, 2);
        if(strcmp("BP101WX1",lcd_model_name) == 0)
        	LCD_POWER_DC1SW_EN(0,0);
    }
...
EXPORT_SYMBOL(lcd_id_used);//gongpiqiang+++20140313
EXPORT_SYMBOL(lcd_id);//gongpiqiang+++20140313
