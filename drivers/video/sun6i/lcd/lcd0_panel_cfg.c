...
#include "lcd_bak/lcd_dp501.h"
#include "lcd_bak/TC358768.h"//gongpiqiang+++
...
static __s32 LCD_open_flow(__u32 sel)
{
  ...
  /* fetch lcd_tcon_fix */
  val.val = 0;
  type = script_get_item("lcd0_para","lcd_tcon_fix", &val);
  if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {  	
    printk("fetch lcd_tcon_fix from sys_config failed\n");
  } else {
  ...
  if(val.val) {
  	LCD_OPEN_FUNC(sel, TCON_open, 100);     //open lcd controller, and delay 100ms
		...
		LCD_OPEN_FUNC(sel, LCD_panel_init, 10);   //open lcd power, than delay 200ms
		LCD_OPEN_FUNC(sel, TCON_open, 200);     //open lcd controller, and delay 100ms ,inet use 100, 50ms delay may causle lcd backlight white when power weak up
		LCD_OPEN_FUNC(sel, LCD_bl_open, 0);     //open lcd backlight, and delay 0ms
	}
	//*/
	/*
...
{	
      __panel_para_t *info = kmalloc(sizeof(__panel_para_t), GFP_KERNEL | __GFP_ZERO);
	if(strcmp("BP101WX1",info->lcd_model_name) == 0) {
		printk("BP101WX1 lcd model used\n");
		LCD_CLOSE_FUNC(sel, LCD_power_off, 100);   //close lcd power, and delay 500ms
		LCD_CLOSE_FUNC(sel, LCD_bl_close, 0);       //close lcd backlight, and delay 0ms
		LCD_CLOSE_FUNC(sel, TCON_close, 100);         //close lcd controller, and delay 0ms
	} else {
	LCD_CLOSE_FUNC(sel, LCD_bl_close, 0);       //close lcd backlight, and delay 0ms
...
static void LCD_panel_init(__u32 sel)
{
    ...
    else if((info->lcd_if == LCD_IF_EDP) && (info->lcd_edp_tx_ic == 2))
    {
         dp501_init(info);
    }//Rocky@20130508-
	else if((info->lcd_if == LCD_IF_EXT_DSI) && (info->lcd_ext_dsi_if == 0))
    {
  	...
  	else if((info->lcd_if == LCD_IF_EXT_DSI) && (info->lcd_ext_dsi_if == 6))//gongpiqiang+++,tc358768
  	{
  			TC358768_init(info);
  	}
