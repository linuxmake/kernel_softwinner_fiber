...
static int lr_change = 0;//gongpiqiang+++ for lr_change
...
static  void codec_init(void)
{
	...
	*/
	...
	//gongpiqiang+++ for l_r change
	val.val = 0;
	type = script_get_item("audio_para", "lr_change", &val);
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
       	printk("[audiocodec] lr_change type err!\n");
    }
  lr_change = val.val;
  script_item_u item_lr;
  type = script_get_item("audio_para", "lr_id", &item_lr);
	if (SCIRPT_ITEM_VALUE_TYPE_PIO != type) {
		printk("script_get_item return type err,no lr_id used\n");
	} else {
		/*request gpio*/
		req_status = gpio_request(item_lr.gpio.gpio, NULL);
		if (0 != req_status) {
			printk("request gpio failed!\n");
		}
		/*config gpio info of audio_pa_ctrl, the default pa config is close(check pa sys_config1.fex).*/
		if (0 != sw_gpio_getall_range(&item_lr.gpio, 1)) {
			printk("sw_gpio_getall_range  item_lr failed\n");
		} else {	
				lr_change = item_lr.gpio.data ? (!lr_change) : lr_change;
		}
	}
	if(lr_change)
		codec_wr_control(SUN6I_DAC_DEBUG,0x1,6,0x1);
	//gongpiqiang--- for l_r change 
}
...
static int codec_pa_play_open(void)
{
	...
	//msleep(162);//Rocky@20130506+
	msleep(62);//如果使用162，在播放视频时，拔出耳机，视频画面会有卡顿
...
*	this function just used for the system voice(such as music and moive voice and so on),
*	no the phone call.  耳机喇叭的切换是用的这个函数 gongpiqiang
*/
